-- Cortex Enterprise — Supabase Schema
-- Persistent memory for enterprise AI assistant
--
-- Run in Supabase SQL Editor or via Supabase MCP.
-- After running, set up Edge Functions for embeddings + search.
...
CREATE TABLE IF NOT EXISTS messages (
  ...
  channel TEXT DEFAULT 'webhook' CHECK (channel IN ('slack', 'teams', 'googlechat', 'webhook')),
...
-- ============================================================
-- MEMORY TABLE (Facts, Goals, Knowledge)
-- ============================================================
CREATE TABLE IF NOT EXISTS memory (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  type TEXT NOT NULL CHECK (type IN ('fact', 'goal', 'completed_goal', 'preference', 'decision')),
  content TEXT NOT NULL,
  deadline TIMESTAMPTZ,
  completed_at TIMESTAMPTZ,
  priority INTEGER DEFAULT 0,
  team TEXT,                    -- which team stored this
  metadata JSONB DEFAULT '{}',
  embedding VECTOR(1536)
);

CREATE INDEX IF NOT EXISTS idx_memory_type ON memory(type);
CREATE INDEX IF NOT EXISTS idx_memory_team ON memory(team);
CREATE INDEX IF NOT EXISTS idx_memory_created_at ON memory(created_at DESC);

-- ============================================================
-- LOGS TABLE (Audit & Observability)
-- ============================================================
...
BEGIN
  RETURN QUERY
  SELECT m.id, m.content, m.deadline, m.priority, m.team
  FROM memory m
  WHERE m.type = 'goal'
...
-- ============================================================
-- SEMANTIC SEARCH (used by Edge Functions)
-- ============================================================
